...
namespace utils::nt
{
	...
	bool library::is_address_in_range(const void* address) const
{
    if (!this->is_valid()) return false;

    auto* const base = this->get_ptr();
    auto* const header = this->get_optional_header();
    if (!header) return false;

    const auto* const start = base;
    const auto* const end = base + header->SizeOfImage;
